...
func _input(event):
	...
	if space_started and not arrows_started and (event.is_action_pressed("up") or event.is_action_pressed("down") or event.is_action_pressed("left") or event.is_action_pressed("right")):
		arrows_started = true
		var t = Tween.new()
